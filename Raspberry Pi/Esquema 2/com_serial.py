...
def tx_serial(dato_tx_serial):
    ...
    print("Envie por serial posicion a la que debe dirigirse AR: %s \n \n" % dato_tx_serial)
...
def rx_socket():
    ...
    while not dato_valido:
        ...
        if (len(dato_rx_socket_cortado)==18):
            dato_valido = True
    print("Recibi por socket posicion a la que debe dirigirse AR: %s \n \n" % dato_rx_socket_cortado)
...
def tx_socket(dato_tx_socket):
    ...
    print("Envie por socket posicion valida donde se encuentra AR: %s \n" % dato_tx_socket)
...
try:	   
    while 1:
        contador += 1
        print(contador)        
        msje_rx_serial, llegue = rx_serial()
        tx_socket(msje_rx_serial)
        msje_rx_socket = rx_socket()
        if (llegue == False):
            tx_serial(msje_rx_socket)
except:
        sys.exit()
